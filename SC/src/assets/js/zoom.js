class Zoom {
	...
	setPointsPosition () {
		...
		let [eleWidth, eleHeight] = this.widthHeight;
		let	positions = [
	...
	addPoints (pointsArr) {
		let ele = this.ele;
		let	cursors = ['nw-resize', 'n-resize', 'ne-resize', 'w-resize', 'e-resize', 'sw-resize', 's-resize', 'se-resize']; // 光标样式
	...
	handlePoints (pointsArr) {
		const that = this;
		const ele = this.ele;
		const mouseDown = (event, point) => {
			let e = event;
			let doc = document;
			let initPoint = [e.clientX, e.clientY]; // 记录下鼠标按下时光标的位置
			let [initWidth, initHeight] = that.widthHeight; // 获取宽度、高度初始值
			let edgeEle = this.edgeEle;
			let [edgeEleWidth, edgeEleHeight] = [edgeEle.clientWidth, edgeEle.clientHeight];
			// 以此元素的父元素为基准进行计算此元素在某些放大情况下的最大宽度、高度。
			let [maxWidth, maxHeight] = [initWidth, initHeight]; // 初始化最大宽度、高度
			let initProportion = parseFloat((initWidth / initHeight).toFixed(2)); // 初始宽高比
			let mouseMove = (point, maxWidth, maxHeight, event) => {
				let e = event;
				let [nowWidth, nowHeight] = (function (e, i) { // 实时计算当前的宽、高的值。
					switch (i) {
					case 0:
				...
				if ([0, 2, 5, 7].includes(point)) { // 四个顶点为等比缩放功能
					if (nowWidth >= maxWidth) { // 当当前宽度已经达到最大值的时候直接计算高度
						...
						let	heightToWidth = nowHeight * initProportion;
